...
# first line loads dataset distmatrixand data set hh which contains demographic information on each household
load('../locationhh.RData')
#===============================================================
# set constants 
endInitial  = 10; endOfBurnIn   = 40  # simple ARMA process endpoint
maxTimeSteps= 160; nIterations   = 800
nhh         = dim(distmatrix)[1]      # nhh is the number of households
...
#===============================================================
# initialize data containers; these containers contain one entry for each household
CCMatrix  = matrix(data=0, nrow = maxTimeSteps, ncol=nhh)       # clinical cases in household  
coverageMatrix  = matrix(data=0, nrow = maxTimeSteps, ncol=nhh) #of neighboring individuals with intervention
aveCCnhood  = matrix(data=0, nrow = maxTimeSteps, ncol=nhh)     # of neighboring individuals with clinical cases
reservoir  = matrix(data=0, nrow = maxTimeSteps, ncol=nhh)      # weighted mean of past AveCCnhood 
boundaryNeighbors  = matrix(data=0, nrow = maxTimeSteps, ncol=nhh)# those on the border at this timestep  
output = matrix(data=0, nrow = maxTimeSteps, ncol = 10 )         # efficacy measures
...
for (iteration in 1:nIterations){   
   # clear containers at the beginning of each iteration
    ...
    # assignment of original disease state. CC is clinical cases per household
        CCMatrix[t,] = rbinom(rep(1,nhh), hh$N, initialPrevalence)
	...
	# output components for effectiveness measures		
        ...
        compare1CC    = sum(CCMatrix [t, ]*comparator1x )
        compare2CC    = sum(CCMatrix [t, ]*comparator2x ) 
